...
const PlantCard: React.FC<PlantCardProps> = ({ plant, selected, canAfford, onClick }) => {
  return (
    <div 
      className={`plant-card p-2 flex flex-col items-center ${selected ? 'ring-2 ring-offset-2 ring-white' : ''} ${!canAfford ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:scale-105 transition-transform'}`}
    ...
    >
      <div className="w-full h-full flex flex-col items-center relative">
        ...
        <div className={`relative w-14 h-14 rounded-full ${plant.color} flex items-center justify-center mb-1 overflow-hidden border-2 border-gray-800 shadow-md`}>
          ...
          {/* Prefer actual image if available, fallback to emoji */}
          {plant.image ? (
            <img 
              src={plant.image} 
              alt={plant.name} 
